...
<body>
    ...
    <script>
        $(document).ready(function() {
            // Fetch and display cart count on page load
            updateCartCount();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response && response.cartCount !== undefined) {
                            ...
                            $('#cart_item_total').text(response.total);
                            if (response.products && response.products.length > 0) {
                                $.each(response.products, function(index, product) {
                                    var productHtml = '<div class="product">';
                                    productHtml += '<div class="product-cart-details">';
                                    productHtml += '<h4 class="product-title"><a href="' +
                                        product.product.product_name + '">' + product.product
                                        .product_name + '</a></h4>';
                                    productHtml += '<span class="cart-product-info">';
                                    productHtml += '<span class="cart-product-qty">' + product
                                        .quantity + ' x ' + product.product.price + '</span>';
                                    productHtml += '</span>'; // Closed span.cart-product-info
                                    productHtml += '</div>'; // Closed div.product-cart-details
                                    productHtml += '<figure class="product-image-container">';
                                    productHtml +=
                                        '<a href="" class="product-image">';
                                    // Adjusted the image source
                                    productHtml += '<img src="{{ asset('uploads/') }}/' +
                                        product.product.images + '" alt="product">';
                                    productHtml += '</a>';
                                    // Concatenate the shopping cart item's ID into the removal link URL
                                    productHtml +=
                                        '<a href="{{ route('removeCartItem', '') }}/' + product
                                        .shoppingCart_id +
                                        '" class="btn-remove" title="Remove Product"><i class="icon-close"></i></a>';
                                    productHtml += '</div>'; // Closed div.product
                            ...
                            console.log('No cart count found in the response.');
                        }
                    },
            ...
            // Update cart count periodically (e.g., every 30 seconds)
            setInterval(updateCartCount, 30000); // 30 seconds
        });
    </script>
